...
import {
  pageTitle, sectionHeader, card, badge, rowDangerButton, input,
} from '../styles/ui'
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
        ...
        <select
          ...
          onChange={e => setFilterPagamento(e.target.value as 'all' | Sale['formaPagamento'])}
      ...
      {isPending ? (
        ...
        <p style={{ color: 'var(--gray)', fontFamily: 'var(--font-body)' }}>Nenhuma venda corresponde aos filtros.</p>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          {/* Header row */}
          <div style={{ display: 'grid', gridTemplateColumns: '150px 100px 1fr 2fr 130px 80px 90px 100px', gap: '1rem', padding: '0 1rem', paddingBottom: '0.5rem' }}>
            {['Data/Hora', 'Pagamento', 'Cliente', 'Itens', 'Operador', 'Total', 'Status', 'Ações'].map(col => (
          ...
          {filteredSales.map(sale => (
            <div key={sale.id} style={{ ...card, opacity: sale.status === 'CANCELLED' ? 0.7 : 1 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '150px 100px 1fr 2fr 130px 80px 90px 100px', gap: '1rem', alignItems: 'center' }}>
                {/* Date/time */}
                ...
                {/* Items */}
                <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.75rem', color: 'var(--gray)', lineHeight: 1.3 }}>
